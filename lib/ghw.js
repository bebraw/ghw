...
function transformers() {
    return attachMissing({
        table: {
            match: function(text, output) {
                ...
                // XXX: can't be after ifs! -> no output
                var ast = {
                    header: headerParts,
            ...
            toHTML: function(o) {
                var h = '<tr><th>' + o.header.join('</th><th>') + '</th></tr>';
                var c = '<tr>' + o.lines.map(function(v) {return '<td>' + v.join('</td><td>') + '</td>'}).join('</tr><tr>') + '</tr>';
        ...
        bracket_pipe_link: {
            ...
            toHTML: function(o) {
                ...
                return '<a href="' + ucFirst(o.r.replace(' ', '-')) + suffix + '">' + o.l + '</a>';
            }
        },
        pipe_link: {
            pattern: /\[([^\|]+)\|([^\]]+)\]/g,
        ...
        bracket_link: {
            pattern: /\[\[([^\]]+)\]\]/g,
            toHTML: function(o) {
                return '<a href="' + ucFirst(o.l.replace(' ', '-'))  + '.html">' + o.l + '</a>';
            }
        },
        paragraphs: {
            toHTML: function(o) {
    ...
    function ucFirst(t) {
        function isLower(a) {return a.toLowerCase() == a;}
        var b = isLower(t[0])? t[0]: t[0].toUpperCase();
        return b + t.slice(1).toLowerCase();
    }
